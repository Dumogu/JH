...
-- EventHandler
-------------------------------------
local function EventHandler(szEvent)  -- 错误信息搜集和反馈
	-- local nTime = GetTime()
...
local _L = GetLang() --将_L作为语言转换函数的简写

local _VERSION_   = 0x1060000   --16位
...
local JH = JH
-- 插件界面 在不打开时 永远不初始化
function JH.OnFrameCreate() --定义并初始化界面函数
	this.bInit = true
	this:RegisterEvent("UI_SCALED") -- 注册界面
	this:RegisterEvent("CALL_LUA_ERROR") -- 注册报错信息
end
